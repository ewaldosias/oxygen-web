...
type Step = 'role' | 'phone' | 'otp' | 'name' | 'specialty' | 'conditions'
...
const COUNTRIES = [
  ...
  { flag:'🇩🇴', name:'Rép. Dominicaine', dial:'+1'   },
...
function useMolCanvas(ref: React.RefObject<HTMLCanvasElement | null>) {
  useEffect(() => {
    ...
    function init() {
      nodes = []
      const c = Math.min(20, Math.floor(W * H / 14000))
      for (let i = 0; i < c; i++) nodes.push({ x: Math.random()*W, y: Math.random()*H, vx: (Math.random()-.5)*.3, vy: (Math.random()-.5)*.3, r: Math.random()*2+1 })
    }
    function tick() {
      ctx!.clearRect(0, 0, W, H); const MAX = 120
...
export default function LoginPage() {
  ...
  async function handleSendOtp() {
    ...
    if (error) {
      ...
      setErrMsg('Erè — verifye nimewo ou eseye ankò')
      return
    }
    setStep('otp')
    let s = 60; setResendTimer(s)
    const iv = setInterval(() => { s--; setResendTimer(s); if (s === 0) clearInterval(iv) }, 1000)
  }

  /* ── CONFIRM OTP ── */
  async function handleConfirmOtp() {
    const code = otp.join('')
    console.log('Confirming OTP:', code, 'for phone:', fullPhone)
    if (code.length < 6) { setErrMsg('Antre 6 chif yo'); return }
    setLoading(true); setErrMsg('')
    ...
    if (error) {
      ...
      setLoading(false)
      setErrMsg('Kòd la pa kòrèk — eseye ankò')
      return
    }

    // Check si utilisateur existant dans la table users
    try {
      ...
      console.log('existing user:', existing, 'dbErr:', dbErr)

      if (existing) {
        // Utilisateur existant → redirect selon rôle
        setLoading(false)
    ...
    // Nouvel utilisateur → continuer le flow
    setLoading(false)
    setStep('name')
  }
    ...
    try {
      ...
      if (role === 'doctor') {
        ...
        await supabase.from('care_profiles').upsert({
          user_id: user.id,
          conditions,
          specialty: conditions.length ? 'medecine_interne' : null,
        }, { onConflict: 'user_id' })
      ...
      router.push(role === 'doctor' ? '/doctor/home' : '/care/home')
    } finally {
      setLoading(false)
    }
  }
  ...
  const stepTotal = role === 'doctor' ? 4 : 5
  const stepIdx   = ['role','phone','otp','name','specialty','conditions'].indexOf(step)
  ...
  return (
    <div style={{minHeight:'100vh',background:`linear-gradient(150deg,#065C50 0%,${TEAL} 100%)`,fontFamily:'DM Sans, sans-serif',color:'white',padding:'0 24px 48px',display:'flex',flexDirection:'column',position:'relative',overflow:'hidden'}}>
      ...
      {/* Header */}
      <div style={{position:'relative',zIndex:2,paddingTop:'60px',paddingBottom:'24px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        {step !== 'role' ? (
          <button onClick={()=>{
      ...
      <div style={{position:'relative',zIndex:2,flex:1,display:'flex',flexDirection:'column',justifyContent:'center'}}>
        ...
        {step === 'phone' && (
          <div className="fu">
            ...
            <button style={{...S.btn,opacity:loading?.7:1}} onClick={handleSendOtp} disabled={loading}>
              {loading ? 'N ap voye kòd la...' : 'Voye kòd la'}
            </button>
          </div>
        )}
        ...
        {step === 'otp' && (
          <div className="fu">
            ...
            <button
              style={{...S.btn, opacity: (otp.join('').length < 6 || loading) ? .5 : 1, marginBottom:'14px'}}
        ...
        {step === 'name' && (
          <div className="fu">
            ...
            <input
              style={{...S.input, fontSize:'22px', marginBottom:'24px'}}
              type="text"
              placeholder={role === 'doctor' ? 'Dr. Jean Pierre' : 'Prenon ou'}
              value={firstName}
              onChange={e => setFirstName(e.target.value)}
              autoFocus
            />
            <button
              style={{...S.btn, opacity: firstName.trim() ? 1 : .5}}
              onClick={() => firstName.trim() && setStep(role === 'doctor' ? 'specialty' : 'conditions')}
                ...
                <div key={s.key} className="chip-press" onClick={()=>setSpecialty(s.key)} style={{background:specialty===s.key?'rgba(212,168,67,.15)':'rgba(255,255,255,.08)',border:`1.5px solid ${specialty===s.key?GOLD:'rgba(255,255,255,.15)'}`,borderRadius:'16px',padding:'18px 20px',display:'flex',alignItems:'center',gap:'14px',cursor:'pointer',transition:'all .2s'}}>
                  ...
                  {specialty===s.key && (
                    <div style={{width:'20px',height:'20px',borderRadius:'50%',background:GOLD,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                      <svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4L3.5 6.5L9 1" stroke="#065C50" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </div>
                  )}
                </div>
              ))}
            </div>
            <button style={{...S.btn, opacity: specialty ? 1 : .5}} onClick={handleFinish} disabled={!specialty||loading}>
              {loading ? '...' : 'Kòmanse OxyGen Care'}
            </button>
          </div>
        )}
        ...
        {step === 'conditions' && (
          <div className="fu">
            ...
            <button style={{...S.btn, opacity: conditions.length&&!loading ? 1 : .5}} onClick={handleFinish} disabled={conditions.length===0||loading}>
              {loading ? '...' : 'Kòmanse OxyGen Care'}
            </button>
          </div>
        )}
